...
(* let cycle (lst : 'a mlist ref) (searched : ('a mlist ref) list)  = *) 
 (*  let found = List.memq lst searched in *)
 (*  let newsearched = lst :: searched in *) 
 (*  found, newsearched;; *) 

let rec cycle (lst : 'a mlist) (searched : ('a mlist ref) list) = 
  match lst with 
  | Nil -> false, searched  
  | Cons (_hd, tl) -> if (List.memq tl searched) = false
...
(* match !lst with *) 
(*   | Nil -> () *)
(*   | Cons(a, b) -> let found, searched = cycle b in *) 
